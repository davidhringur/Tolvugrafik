...
window.onload = function init()
{
    ...
    canvas.addEventListener("mousemove", function(e){
        if(movement) {
    	    spinY = ( spinY + (e.offsetX - origX)*0.05 ) % 360 ///;
            spinX = ( spinX + (e.offsetY - origY)*0.05 ) % 360 ///360 * 2 * Math.PI;
            origX = e.offsetX;
            origY = e.offsetY;
        }
    } );
...
function render()
{
	...
	case 3:
	    ...
	    mv = lookAt( vec3(150.0, 0.0, 5.0+height), vec3(carXPos, carYPos, 0.0), vec3(0.0, 0.0, 1.0 ) );
	    drawScenery( mv );
	    mv = mult( mv, translate(carXPos, carYPos, 0.0) );
	    mv = mult( mv, rotateZ( carDirection ) ) ;
	    drawCar( mv );
    ...
    case 9:
        // Static viewpoint outside the track; camera follows car
        mv = lookAt( vec3(-40.0, 140.0, 15+height), vec3(carXPos, carYPos, 0.0), vec3(0.0, 0.0, 1.0 ) );
        drawScenery( mv );
        mv = mult( mv, translate(carXPos, carYPos, 0.0) );
        mv = mult( mv, rotateZ( carDirection ) ) ;
        drawCar( mv );
