...
function render()
{
    ...
    mv1 = mult( mv1, scalem( 0.2, 0.02, 0.3 ) );
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );

    // Finally the middle bar (no translation necessary)
    mv1 = mult( mv, rotateX(-Math.sin(time/30)*30) );
    mv1 = mult( mv1, translate( 0, -0.02, 0.0 ) );
    mv1 = mult( mv1, scalem( 0.1, 0.03, 1.0 ) );
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );
