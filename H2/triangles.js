...
var maxNumPoints = 200*3;       // H�marksfj�ldi punkta sem forriti� r��ur vi�!
...
window.onload = function init() {
    ...
    canvas.addEventListener("click", function(e){
        ...
        var size = 25/canvas.width
        var t = [vec2(2*e.offsetX/canvas.width-1-size, 2*(canvas.height-e.offsetY)/canvas.height-1-size),
                 vec2(2*e.offsetX/canvas.width-1, 2*(canvas.height-e.offsetY)/canvas.height-1+size),
                 vec2(2*e.offsetX/canvas.width-1+size, 2*(canvas.height-e.offsetY)/canvas.height-1-size)   ];
